...
###### USER NOTES:
# This code is provided by Eric C. J. Oliver

# This script is not meant to be runned
...
def detect(t, temp, climatologyPeriod=[None,None], pctile=90, windowHalfWidth=5, smoothPercentile=True, smoothPercentileWidth=31, minDuration=5, joinAcrossGaps=True, maxGap=2, maxPadLength=False, coldSpells=False, alternateClimatology=False, Ly=False, splitYearOverlappingMHWs=False):
    '''
    ...
    ## BREIZHCODE START
    # Here is the place to split events if spanning on multiple years



        # return date.toordinal(date(year, month, day))

    # time_starts = []
    # time_ends = []

    # # Split events if spanning on multiple years
    ...
    #     # Not spanning, append as is
    #     if year_start == year_end:
    #         time_starts.append(mhw['time_start'][ev])
    #         time_ends.append(mhw['time_end'][ev])
        
    #     # Spanning, splitting the MHW event
    #     else:
    #         time_starts.append(mhw['time_start'][ev])
    #         time_ends.append(ordinal_time(year_start, 12, 31))

    #         for i in range(year_end - year_start - 1):
    #             year = year_start + i + 1
    #             time_starts.append(ordinal_time(year, 1, 1))
    #             time_ends.append(ordinal_time(year, 12, 31))
            
    #         time_starts.append(ordinal_time(year_end, 1, 1))
    #         time_ends.append(mhw['time_end'][ev])

    # mhw['time_start'] = time_starts
    # mhw['time_end'] = time_ends


    
    ## THIS CODE IS ADDED TO INITIAL ##

    if splitYearOverlappingMHWs:
        ...
        while ev < len(mhw['time_start']):
            ...
            if year_start < year_end:
                ...
                #  -> Ending this event on 31 December of its year
                ...
                # while not new_end_time in t:
                #     new_end_time -= 1

                # # Handle missing values cases
                # while not next_start_time in t:
                #     next_start_time += 1
    ...
    for ev in range(mhw['n_events']):
        ...
        if len(np.where(t==mhw['time_end'][ev])) < 1 or len(np.where(t==mhw['time_end'][ev])[0]) < 1:
            raise Exception(f"Ay, {mhw['date_end'][ev]} ({mhw['time_end'][ev]}) not found.")
